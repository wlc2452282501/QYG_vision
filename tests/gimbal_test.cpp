...
int main(int argc, char * argv[])
{
  ...
  while (!exiter.exit()) {
    ...
    auto fired = false;  // bullet_count field removed
    ...
    data["q_pitch"] = ypr[1];
    data["yaw"] = state.yaw;
    // data["vyaw"] = state.yaw_vel;  // field removed
    data["pitch"] = state.pitch;
    // data["vpitch"] = state.pitch_vel;  // field removed
    data["bullet_speed"] = state.bullet_speed;
    // data["bullet_count"] = state.bullet_count;  // field removed
